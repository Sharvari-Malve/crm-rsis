    ...
    const fetchStats = async () => {
      try {
        ...
        const leadRes = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/get-lead-count`
        );

        // ✅ Fetch total follow ups
        const followRes = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/get-total-follow-up`
        );
        ...
        setStats({
          leads: { total: leadData?.totalLeads || 0 },
          followUps: { total: followData?.totalFollowUps || 0 },
        });
      } catch (error) {
