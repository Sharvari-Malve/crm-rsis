...
// kjkjkj
  ...
  const handleConfirmStatusToggle = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        {
          id: statusUser.id,
          status: statusUser.status === "enable" ? "disable" : "enable"
        },
        { headers: { Authorization: `Bearer ${cookies.get("auth")}` } }
      );
  ...
  const handleStatusToggleConfirmation = (user: User) => {
    setStatusUser(user);         // set selected user
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 w-full body-scroll">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-2 w-full">
        {/* Title & Subtitle */}
        ...
        <div className="flex flex-col sm:flex-row md:items-center gap-3 w-full sm:w-auto">
          {/* Search */}
          <div className="relative flex-1 sm:flex-none w-full sm:w-60 md:w-32">
            ...
            {currentUsers.map((user, index) => (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 sm:px-6 py-3 flex items-center space-x-2 sm:space-x-3">
                  ...
                  <button onClick={() => handleDeleteUser(user)} className="text-teal-600 hover:text-teal-700">
                    <Trash2 size={20} />
                  </button>
                  ...
                  <div key={user.id} className="flex items-center justify-between">
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={() => handleStatusToggleConfirmation(user)}
                      />
                      <div className="w-10 h-5 bg-gray-200 rounded-full peer peer-checked:bg-teal-600 relative
                            after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:w-4 after:h-4
      ...
      {showDeleteModal && deleteUser && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md sm:max-w-lg shadow-2xl border border-gray-300 rounded-xl">
            <div className="bg-teal-600 px-6 py-4 rounded-t-xl">
            ...
            <div className="px-6 py-4 border-t bg-gray-50 flex justify-end space-x-3 rounded-b-xl">
              ...
              <button onClick={confirmDeleteUser} className="px-5 py-2 bg-teal-600 hover:bg-red-700 text-white font-semibold rounded-lg text-sm sm:text-base">Delete</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded shadow-md w-full max-w-sm">
            <h2 className="text-lg font-semibold mb-4">
              Are you sure you want to {statusUser.status === "enable" ? "disable" : "enable"} this user?
            </h2>
